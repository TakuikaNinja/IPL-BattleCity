...
ZeroPage_Offset:.BYTE 0	; (uninited)	; DATA XREF: ROM:Zero_Page_Viewerr
					...
					; Смещение от начала нулевой страницы
		.BYTE 0	; (uninited)
		.BYTE 0	; (uninited)
		.BYTE 0	; (uninited)
		.BYTE 0	; (uninited)
...
Zero_Page_Viewer:			; Не используется в игре (позволяет просматривать zero page memory)
		LDA	ZeroPage_Offset
		JSR	Num_To_NumString ; Переводит число из А	в строку NumString
		LDA	#$30 ; '0'
		STA	Char_Index_Base	; Цифры
		LDA	#0
		STA	HighPtr_Byte
		LDA	#Num_String+4
		STA	LowPtr_Byte
		LDX	#9
		LDY	#2		; Координаты надписи
		JSR	Save_Str_To_ScrBuffer ;	Сохраняет строку в строковый буффер
		LDX	ZeroPage_Offset	; Смещение от начала нулевой страницы
		LDA	0,X		; Отображаем содержимое	Zero Page
		JSR	Num_To_NumString ; Переводит число из А	в строку NumString
		LDA	#0
		STA	HighPtr_Byte
		LDA	#Num_String+4
		...
		INC	ZeroPage_Offset	; Показываем следующий байт

SkipInc_Zero_Page_Viewer:		; CODE XREF: ROM:C78Dj
		LDA	Joypad1_Differ
		AND	#2		; B?
		BEQ	ScipDec_Zero_Page_Viewer
		DEC	ZeroPage_Offset	; Показываем предыдущий	байт

ScipDec_Zero_Page_Viewer:		; CODE XREF: ROM:C796j
		LDA	Joypad1_Differ
		AND	#1		; A?
		BEQ	End_Zero_Page_Viewer
		LDA	ZeroPage_Offset	; Смещение от начала нулевой страницы
		CLC
		ADC	#$10		; Отображаем содержимое	через $10 байт
		STA	ZeroPage_Offset	; Смещение от начала нулевой страницы
...
Skip_BonusHandle_TankImpact:		; CODE XREF: BulletToTank_Impact_Handle+C9j
		...
		DEC	Tank_Type,X
		LDA	#1
		STA	Snd_ArmourRicochetTank ; Танк бронирован
		JMP	Next_Bullet2_Tank_Impact
; ───────────────────────────────────────────────────────────────────────────
...
-:					; CODE XREF: Sound_Stop+2Aj
		TYA
		STA	(Low_Ptr_Snd),Y
		STA	Snd_Pause,X	; Останавливаем	все звуки
		CLC
		LDA	Low_Ptr_Snd
		ADC	#8
		STA	Low_Ptr_Snd
